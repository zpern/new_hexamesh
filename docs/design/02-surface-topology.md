...
Plan 2 为混合三角形/四边形 `SurfaceMesh` 构建稳定、只读且可验证的表面拓扑。
...
拓扑构建包含非平凡实现，不放入公开头文件。Plan 2 将 `boundary_mesh_core` 从纯 `INTERFACE` 目标改为包含 `.cpp` 的静态库，同时保留公开别名：
...
该文件当前位于构建目录，且新工程尚未实现正式 PLY 读取模块。Plan 2 不增加临时读取器，也不依赖该路径。等 IO 阶段完成后，再将它登记为大型集成与性能回归案例。
