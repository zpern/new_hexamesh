...
终止 Quad 在物化前具有稳定的 `InternalSplit / ExternalPatch / KeepHexa`
决策。长宽比只决定尝试顺序：通过预筛时先内部后外置，不通过时先外置后
内部；两条路径都失败才保留 Hexa 并输出 error。外置候选从默认距离开始，若
相交则持续减半至最小比例 `1e-6`，建立无碰撞/碰撞区间后再二分选择最大安全
距离。达到该下限仍相交时不再缩小，而是进入 high-face rollback 或内部切割
回退。`ExternalPatch`
保留原 Hexa 并只追加外侧 Pyramid，不再执行七单元内部切割。最终器不得自行
生成未经 resolver 联合相交检查的外置 patch。

达到用户请求层数上限的候选 Quad 也在该层提交前进入同一 resolver。即使本层
没有普通高低面过渡，也要用当前底面和候选顶面计算长宽比及内部可行点，并把
外置 patch 与原始表面、历史外露边界和同层候选一起检查后保存稳定决策；不得
因为 `transition_low_faces` 为空而跳过该流程。

所有侧向 Triangle/Quad 模板也在临时阶段按固定节点顺序检查 Tetra/Pyramid
的全部子四面体。非正体积触发其依赖高面的单调 rollback 和完整重构；禁止按
体积符号换点或取绝对值。
