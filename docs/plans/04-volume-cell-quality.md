...
- [x] **Step 4: 实现最小修复并删除全部 Jacobian 残留**
...
- Debug CTest：22/22 通过。
- Release CTest：22/22 通过。
- Release benchmark：200 万候选约 1.29 秒，约 155 万 candidates/s；结果仅作为当前机器参考，不设置门槛。
- `include/`、`src/`、`tests/` 中旧 Jacobian、形函数采样和高斯积分残留扫描无输出。
