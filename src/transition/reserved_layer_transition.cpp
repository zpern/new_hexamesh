...
namespace boundary_mesh
{
                 ...
                 ++face_index)
            {
                ...
                if (edges.size() == 2)
                    output[face_index].second_high_edge_local_index =
                        edges[1];
                if (edges.empty() &&
                    output[face_index].layers.trial_layers > 0)
                {
                    std::vector<std::size_t> continuing_corners;
                    std::visit(
                        [&](const auto &face)
                        {
                            for (std::size_t local = 0;
                                 ...
                                 ++local)
                            {
                                const auto uses = vertex_uses.find(
                                    face.vertex_ids[local]);
                                if (uses == vertex_uses.end())
                                    continue;
                                const bool continuing = std::any_of(
                                    uses->second.begin(),
                                    uses->second.end(),
                                    [&](const std::size_t incident)
                                    {
                                        return incident != face_index &&
                                            output[incident]
                                                    .layers.trial_layers >
                                                output[face_index]
                                                    .layers.trial_layers;
                                ...
                                if (continuing)
                                    continuing_corners.push_back(local);
                            }
                            if (continuing_corners.size() != 2)
                                return;
                            const std::size_t first =
                                continuing_corners[0];
                            const std::size_t second =
                                continuing_corners[1];
                            const std::size_t count =
                                face.vertex_ids.size();
                            if ((first + 1) % count == second)
                                output[face_index]
                                    .continuing_edge_local_index = first;
                            else if ((second + 1) % count == first)
                                output[face_index]
                                    .continuing_edge_local_index = second;
                        },
                        front.faces[face_index]);
                }
    ...
    coordinateTransitionFront(
        ...
        const RegularLayerGrowthResult &trial)
    {
        return coordinateFront(front, trial);
    }
                ...
                [&](const auto &face) -> TransitionTemplateResult
                {
                    ...
                    if constexpr (std::is_same_v<Face, Triangle>)
                    {
                        ...
                        input.continuing_edge_local_index =
                            state.continuing_edge_local_index;
